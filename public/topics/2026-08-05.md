...
- **Prime Agent is an open RLM coding harness that can rewrite itself** - [[Prime Agent]] is Prime Intellect's open-source coding / long-horizon agent: a persistent IPython REPL where context lives as a variable, sub-agents are `await rlm(...)` calls, and a Continual Harness lets the agent CRUD its own prompts, skills, memory, and sub-agents. For an [[Recursive Language Models|RLM]] 101, start with Alex Zhang's explainer, then the paper, then Prime Intellect's own RLM writeup.
