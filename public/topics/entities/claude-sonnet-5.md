---
title: Claude Sonnet 5
created: 2026-07-08
updated: 2026-07-08
type: entity
tags: [entity, model, coding-agents]
sources: [raw/articles/2026-07-08-link-records.md, raw/articles/2026-06-24-link-records.md]
---

# Claude Sonnet 5

Claude Sonnet 5 is Anthropic's executor-side agent model in the July 8 board, useful to separate from [[Claude Fable/Mythos]] because the meetup framing treats Sonnet as the cheaper worker and Fable as the advisor/escalation model.

## Mentioned In

- [[Austin AI Club - July 8, 2026]]: **Closed model releases**.
- [[Austin AI Club - June 24, 2026]]: **AI Twitter's model-release calendar slips**.

## Related

- [[Anthropic]]
- [[Claude Fable/Mythos]]
- [[Claude Code]]
- [[Coding Agents]]
- [[Agent Cost Controls]]
