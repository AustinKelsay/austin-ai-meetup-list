...
### Big Tech Moves
- **OpenAI gets its own inference chip** - [[OpenAI]] and [[Broadcom]] unveiled Jalapeno, a custom LLM [[Inference Optimization|inference accelerator]] with lab samples running GPT-5.3-Codex-Spark and a multi-generation gigawatt-scale deployment plan. The play is cost-per-token control: frontier labs want to own the silicon path instead of renting GPU clusters.
