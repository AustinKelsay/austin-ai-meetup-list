...
Related wiki pages: [[Local Builds & Projects]], [[Agent Infrastructure]], [[Models & Research]], [[Security]], [[Big Tech Moves]], [[AI Benchmarks & Evals]], [[AI Signal Discovery]], [[Agent Payments]], [[Inference Optimization]], [[Physical AI]], [[Agent Cost Controls]], [[Coding Agents]], [[Open Models]], [[World Models]], [[Local AI]], [[Compute Strategy]], [[Privacy Tooling]], [[AI Governance]], [[Apple]], [[Anthropic]], [[Andrej Karpathy]], [[Ben Carman]], [[Cohere]], [[Microsoft]], [[MiniMax]], [[OpenAI]], [[xAI]], [[Google]], [[NVIDIA]], [[SpaceX]], [[Claude Code]], [[Gemma]], [[TMax]], [[Claude Fable/Mythos]], [[Grok]], [[Nemotron]], [[DiffusionGemma]].
...
- **Claude gets its cloud-bill moment** - Axios says an unnamed client allegedly spent $500M in a month after failing to cap employee Claude licenses, while The Verge says [[Microsoft]] is winding down most [[Claude Code]] licenses partly for financial reasons. The takeaway is not "guess the company"; it is that agentic AI needs [[Agent Cost Controls]] and [[Agent Payments]]: budgets, routing, per-user limits, and token observability before the bill becomes the incident.
...
- **Local AI becomes bandwidth math** - The question for local AI deployment is no longer just "can it run?" but whether the memory path can keep up with model demands. Fresh bandwidth comparisons show the hardware reality behind local [[Inference Optimization|inference planning]].
  Post: https://x.com/The_Only_Signal/status/2060321842716365147
- **Vector compression hits 10x without the quality tax** - Shard KV cache compression, TurboVec, and TurboQuant all crossed the same threshold this year: aggressive [[Inference Optimization|compression]] that actually works in production. Shard gets 10-11x memory reduction on Llama with zero NIAH degradation, though Tim Dettmers called out the TurboQuant comparison as non-reproducible. Tom Turney shipped TurboQuant+ in llama.cpp within 36 hours of the Google paper, and Ryan Codrai's TurboVec fits 31GB of embeddings into 4GB on a MacBook. The real story is compression moving from research to practical local deployment.
...
- **Open-weight release week turns into a firehose** - The open slide is now a stack, not a list: [[Nemotron]]/Cosmos/OmniDreams for [[Physical AI|physical AI]], [[Gemma]]/[[MiniMax]]/Liquid/Step/OpenBMB/Tencent/[[Cohere]] for local LLMs and coding agents, [[DiffusionGemma]] for text diffusion, PaddleOCR/LocateAnything for vision, MOSS/Higgs/dots for audio, Ideogram/Magenta/NAVA/Lance/Bernini/JoyAI for media, [[TMax]] for terminal-agent RL, WALL-OSS for robotics, and Unsloth/Comfy/NVFP4 quants for [[Inference Optimization|deployment]]. This is also an [[AI Signal Discovery|AI-source triage problem]] now.
...
- **World models need a taxonomy, not another demo** - Fei-Fei Li splits the overloaded [[World Models]] label into renderers, simulators, and planners. That is the cleanest way to compare video generators, physics simulators, [[Physical AI|robotics policies]], and agent planning without pretending they are the same capability.
...
### Security
- **Frontier model review gets a federal lane** - Trump's June 2 AI order creates a classified cyber [[AI Benchmarks & Evals|benchmark]] and a voluntary 30-day pre-release access path for covered frontier models, while explicitly avoiding mandatory licensing. The trust boundary is now government early access, not just lab evals.
...
- **Apple stretches PCC onto Google/NVIDIA** - The sharper Siri infra story is that [[Apple]] now says AFM Cloud Pro extends Private Cloud Compute to [[NVIDIA]] GPUs in [[Google]]'s cloud. If the privacy guarantees hold, PCC just changed from an Apple-Silicon-only cloud story into a verifiable confidential-[[Inference Optimization|inference architecture]] that can rent frontier GPU scale.
...
- **Anthropic enters IPO mode at $965B** - A $65B Series H, $47B run-rate, and confidential S-1 in the same week as Opus 4.8 makes the bet explicit: [[Anthropic|Claude's]] enterprise agent revenue is being priced like a near-trillion public-market company.
