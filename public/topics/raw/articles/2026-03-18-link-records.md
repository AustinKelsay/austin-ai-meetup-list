---
title: March 18, 2026 Source Link Records
created: 2026-05-05
updated: 2026-05-05
type: summary
tags: [source-record, meetup]
sources: []
---

# March 18, 2026 Source Link Records

Public link records for [[Austin AI Club - March 18, 2026]]. These records preserve source metadata and short curation notes without copying full third-party text into the public repo.

## Agent Infrastructure

### Agent Overload

- Agent overload repos:
  - https://github.com/openclaw/openclaw
  - https://github.com/NVIDIA/NemoClaw
  - https://github.com/sipeed/picoclaw
  - https://github.com/nullclaw/nullclaw
  - https://github.com/qwibitai/nanoclaw
  - https://github.com/microclaw/microclaw
  - https://github.com/NousResearch/hermes-agent
  - https://github.com/QwenLM/Qwen-Agent
  - https://github.com/badlogic/pi-mono

### CLI vs MCP

- CLI and MCP records:
  - https://www.scalekit.com/blog/mcp-vs-cli-use
  - https://x.com/ravibits/status/2031807480448389375

### OpenAI WebSockets

- Realtime records:
  - https://developers.openai.com/api/docs/guides/realtime-websocket
  - https://developers.openai.com/api/docs/guides/realtime-conversations
  - https://developers.openai.com/api/docs/guides/realtime-models-prompting
  - https://developers.openai.com/api/docs/guides/realtime-server-controls
  - https://developers.openai.com/api/docs/guides/realtime-transcription
  - https://openai.github.io/openai-agents-js/guides/voice-agents/quickstart/

### Frontier lab agent frameworks

- Frontier lab agent records:
  - https://agentclientprotocol.com/overview/introduction
  - https://platform.claude.com/docs/en/agent-sdk/quickstart

### Claude Code + Codex updates roundup

- Claude Code and Codex records:
  - https://openai.com/index/introducing-the-codex-app/
  - https://openai.com/index/introducing-gpt-5-3-codex/
  - https://docs.anthropic.com/en/docs/claude-code/overview
  - https://docs.anthropic.com/en/docs/claude-code/sub-agents

### Claude distillation claims

- Distillation records:
  - https://www.tomshardware.com/tech-industry/artificial-intelligence/anthropic-accuses-deepseek-other-chinese-ai-developers-of-industrial-scale-copying-claims-distillation-included-24-000-fraudulent-accounts-and-16-million-exchanges-to-train-smaller-models
  - https://www.pcgamer.com/software/ai/anthropic-says-it-has-identified-thousands-of-fraudulent-accounts-taking-claude-and-extracting-its-capabilities-to-train-and-improve-their-own-models/
  - https://www.business-standard.com/technology/tech-news/anthropic-accuses-deepseek-minimax-of-stealing-data-from-ai-model-claude-126022400092_1.html

### OpenClaw pairing bench

- Pairing records:
  - https://x.com/moritzkremb/status/2030221468244775131

## Models & Research
### Closed model releases
- GPT-5.4 mini records:
  - https://x.com/OpenAI/status/2033953592424731072
  - https://openai.com/index/introducing-gpt-5-4-mini-and-nano/

- Xiaomi MiMo record:
  - https://x.com/ArtificialAnlys/status/2034239267052896516

- MiniMax records:
  - https://x.com/MiniMax_AI/status/2034315320337522881
  - https://www.minimax.io/news/minimax-m27-en

### Open model releases
- Qwen records:
  - https://huggingface.co/collections/Qwen/qwen35
  - https://artificialanalysis.ai/leaderboards/models?is_open_weights=open_source

- Nemotron records:
  - https://huggingface.co/collections/nvidia/nvidia-nemotron-v3

- VoiceChat record:
  - https://x.com/ArtificialAnlys/status/2033642073052868861

- Mistral record:
  - https://x.com/MistralDevs/status/2033654167395357082

### LMfit

- LMfit record:
  - https://x.com/dr_cintas/status/2029272137488580761

### Frontier check

- Frontier check record:
  - https://artificialanalysis.ai


## Security

### KeepAI

- KeepAI record:
  - https://x.com/ArturBrugeman/status/2032450145527148575

### SEO Injection

- SEO Injection record:
  - https://x.com/isabellasg3/status/2029710987008315620

### Security nightmare thread

- Security nightmare thread:
  - https://x.com/HedgieMarkets/status/2029337090844946791

## Big Tech Moves

### Meta's "Moltbook"

- Meta's "Moltbook" records:
  - https://techcrunch.com/2026/03/10/meta-acquired-moltbook-the-ai-agent-social-network-that-went-viral-because-of-fake-posts/
  - https://apnews.com/article/31af42ccbb04001dd17a3fc7067d1de3

### Claude Kill?

- Claude Kill? records:
  - https://apnews.com/article/d4608c7dd139245ac8ad94d5427c505a
  - https://openai.com/index/our-agreement-with-the-department-of-war/
  - https://www.axios.com/2026/03/16/tech-industry-rallies-anthropic-pentagon-fight

### Niantic turns Pokemon Go city data into robot navigation

- Niantic spatial records:
  - https://www.techrepublic.com/article/news-coco-robots-niantic-mapping/
  - https://x.com/nianticspatial/status/2031383333221880051
  - https://www.nianticspatial.com/blog/coco-robotics
  - https://gizmodo.com/niantic-partnership-food-delivery-bots-use-pokemon-go-data-2000732212

### MacBook Pro M5 Max as a local inference laptop

- MacBook Pro M5 Max records:
  - https://x.com/aakashgupta/status/2028909261540270085
  - https://www.apple.com/newsroom/2026/03/apple-debuts-m5-pro-and-m5-max-to-supercharge-the-most-demanding-pro-workflows/

### Confer + Meta private AI

- Confer and Meta private AI records:
  - https://x.com/moxie/status/2034096735619059966
  - https://confer.to/blog/2026/03/encrypted-meta/

### NemoClaw launch + OpenClaw reaction

- NemoClaw and OpenClaw reaction records:
  - https://youtu.be/kRmZ5zmMS2o?si=RgD8HNueWbZtK_Td
  - https://x.com/kanavtwt/status/2033835300653568159

## Local Builds & Projects

### pika chat

- pika chat record:
  - https://github.com/sledtools/pika/tree/master

### plebdev-bench

- plebdev-bench records:
  - https://github.com/AustinKelsay/plebdev-bench
  - https://plebdev-bench-dashboard.vercel.app/#/leaderboard

### sage

- sage record:
  - https://github.com/AnthonyRonning/sage

### visibible

- visibible records:
  - https://x.com/bitcoinplebdev/status/2035102011746644051
  - https://x.com/bitcoinplebdev/status/2036172958964535544
  - https://primal.net/p/nprofile1qqswamncyflsf6hqt5w0xwxm7ww5v2hnvqufhrgfeaahkvkjz65hd0cd2xzj0

### plebdev's adversarial AI course

- adversarial AI course record:
  - https://x.com/pleb_devs/status/2028582994596778014
